const ARCHIVE_MANIFEST = "./archive/fightposters.json";
const DEMO_MANIFEST = "./demo/fightposters.json";
...
async function loadManifest() {
  ...
  throw new Error("No FIGHTPOSTERS manifest found.");
}
...
function currentCrop() {
  ...
  // The artwork IS the viewport. Use cover instead of contain so there are
  // never black letterbox borders around the painting.
  ...
  const sourceWidth = Math.min(
    art.width,
    viewportWidth / (fitScale * zoom)
  );

  const sourceHeight = Math.min(
    art.height,
    viewportHeight / (fitScale * zoom)
  );
...
function drawOverview(crop) {
  ...
  const overviewScaleX =
    overviewImage.naturalWidth / currentArtwork.width;

  const overviewScaleY =
    overviewImage.naturalHeight / currentArtwork.height;

  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = "high";

  // Always draw the crop edge-to-edge. No letterboxing / black border.
  ctx.drawImage(
    overviewImage,
...
function tilePath(art, level, col, row) {
  return resolveAsset(
    `${art.tiles}/${level.id}/${col}_${row}.jpg`
  );
}
...
function updateNavigator(crop) {
  ...
  // Navigator directly matches the artwork aspect ratio.
  navigatorWindow.style.left =
    `${(crop.x / currentArtwork.width) * 100}%`;

  navigatorWindow.style.top =
    `${(crop.y / currentArtwork.height) * 100}%`;

  navigatorWindow.style.width =
    `${(crop.width / currentArtwork.width) * 100}%`;

  navigatorWindow.style.height =
    `${(crop.height / currentArtwork.height) * 100}%`;
}
...
function endTouch(event) {
  ...
  // Match desktop behaviour: once the touch ends, zoom back out
  // and recentre the artwork instead of leaving the detail view stuck.
  targetZoom = 1;
  targetFocusX = .5;
  targetFocusY = .5;
  scheduleRender();
...
async function init() {
  try {
    ...
    console.error(error);
    status.textContent = "FIGHTPOSTERS ARCHIVE COULD NOT BE LOADED.";
  }
}
